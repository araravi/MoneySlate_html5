...
function getGroups(){
	...
	if(navigator.onLine){
		...
		{
			groupList += "<li><a href=\"groupUpdate.php?group_id="+groups[i].group_id+ "\">" + groups[i].group_name + "</a></li>";
		}
...
$( '#myGroups' ).live( 'pageinit',function(event){
...
function setMe(){
	...
	var url = "http://ec2-46-137-231-62.ap-southeast-1.compute.amazonaws.com/api/meetile.php?month="+month+"&year="+year;
	var amt;
	$.getJSON(url, function(json) {
			amt = json.amount;
...
function setReport(){
	...
	var url = "http://ec2-46-137-231-62.ap-southeast-1.compute.amazonaws.com/api/reporttile";
	var needAmt;
	var oweAmt;
	var total;
	var needPercent;
...
function setCategories(){
        var url = "http://ec2-46-137-231-62.ap-southeast-1.compute.amazonaws.com/api/getCategory";
	var optionList = "";
	$.getJSON(url, function(json) {
...
function addcategory(cat){
	var url = "http://ec2-46-137-231-62.ap-southeast-1.compute.amazonaws.com/api/addCategory.php";
...
{
	url = 'http://ec2-46-137-231-62.ap-southeast-1.compute.amazonaws.com/api/expense_det/' + expense_id;
	$.getJSON(url, function(json){
...
$( '#addExpensePage' ).live( 'pageinit',function(event){
	...
	$(this).bind("swiperight", function(){
		  $.mobile.changePage( "http://ec2-46-137-231-62.ap-southeast-1.compute.amazonaws.com/home.php");
	});
	...
	$("#submitExpense").bind('click',function(){	
			...
			for(i=0;i<totalSelected;i++){
				...
				if($('#'+member_id+'-amt').length==0)
					...
					expSplit['mem_amount'] = $('#'+member_id+'-amt').val();
				}	
				mem_exp[i]=expSplit;
			}
			...
			$.ajax({
			      type: "PUT",
			      url: 'http://ec2-46-137-231-62.ap-southeast-1.compute.amazonaws.com/api/putExpense.php',
			      data: expenseData,
...
$('#reportPage').live( 'pageinit',function(event){
	function populateHeaders(need,owe){
        var url = "http://ec2-46-137-231-62.ap-southeast-1.compute.amazonaws.com/api/report";
	$.getJSON(url, function(json) {
	   var element = '';
	   var oweTag;
	   var progressTag;
	   ...
	   $.each(json.report, function(i,person){
		...
		element = element+'<li><a style="white-space:normal" href="http://ec2-46-137-231-62.ap-southeast-1.compute.amazonaws.com/api/app0.1/reportdetails.php?mem_id=' + person.mem_fb_id + '&mem_name='+person.first_name+'"><div class="ui-grid-a"><div class="ui-block-a" style="width:20%">'+ image + '</div>';
...
function setPayment(){
	var url = "http://ec2-46-137-231-62.ap-southeast-1.compute.amazonaws.com/api/addPaymentlist";
...
$(this).bind("swipeleft", function(){
	...
	if(paySel==0)
		$.mobile.changePage( "http://ec2-46-137-231-62.ap-southeast-1.compute.amazonaws.com/api/app0.1/userreport.php");
	else
		$.mobile.changePage( "http://ec2-46-137-231-62.ap-southeast-1.compute.amazonaws.com/api/app0.1/reportdetails.php?mem_id="+paySel+"&mem_name="+payName);
	});	
...
$('#paySubmit').bind('click',function(){
	...
	if(navigator.onLine){
		...
		$.ajax({
			  type: "PUT",
			  url: 'http://ec2-46-137-231-62.ap-southeast-1.compute.amazonaws.com/api/addPayment.php',
			  data: payData,
			  contentType: 'application/json', // format of request payload
			  dataType: 'json', // format of the response
			  success: function(msg) {
...
function populateMe(mon,yr){
   ...
   if(yr!="12")
   var url = "http://ec2-46-137-231-62.ap-southeast-1.compute.amazonaws.com/api/getMee.php?sd="+yr+"-"+mon+"-1&ed="+yr+"-"+temp_mon+"-1";
   else
   var url = "http://ec2-46-137-231-62.ap-southeast-1.compute.amazonaws.com/api/getMee.php?sd="+yr+"-"+mon+"-1&ed="+temp_year+"-1-1";
   console.log(url);
   $.getJSON(url, function(json) {
   var element = '';
   var colorTag;
...
function checkAlerts(){
	var url = "http://ec2-46-137-231-62.ap-southeast-1.compute.amazonaws.com/api/feed_check.php";
	if(navigator.onLine){
		$.getJSON(url, function(json){
		msg=(json.msg)
		if(json.msg==true)
