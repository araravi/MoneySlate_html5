...
<script>
	...
	function set_friends(searchTag){
		...
		$.getJSON(url, function(friends){
			...
			{	if($('#'+friends[i].uid+'-rem').length!=0){
				...
				friendsList += '<li data-icon="plus" id="'+friends[i].uid+'-ls"><a href=# id='+friends[i].uid+'>'+friends[i].name+'</a></li>';
				
				$('#'+friends[i].uid).live('click',function(){
